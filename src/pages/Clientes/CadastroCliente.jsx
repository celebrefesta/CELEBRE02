...
const CadastroCliente = () => {
  ...
  const clienteEditando = location.state?.clienteEditando ||
null;
...
// 🔥 ESTADOS PARA O HISTÓRICO DO CLIENTE 🔥
  ...
  useEffect(() => {
    ...
    if (clienteEditando) {
      ...
      const statusReal = clienteEditando.statusCadastro ? clienteEditando.statusCadastro : 
(eraPendenteAntigo ? 'pendente' : 'aprovado');
...
'', inscricaoEstadual: clienteEditando.inscricaoEstadual || '',
...
'', telefoneFixo: clienteEditando.telefoneFixo || '', 
...
'', complemento: formatarNomeCapitalizado(clienteEditando.complemento || ''), 
        ...
        tags: clienteEditando.tags ||
'NOVO', 
        observacoes: clienteEditando.observacoes ||
'',
        situacaoFinanceira: clienteEditando.situacaoFinanceira ||
'adimplente',
...
= await getDocs(qLocacoes);
  ...
  const valorTotalLoc = Number(loc.valorTotal || loc.total || 0);
            ...
            if (!statusLoc.includes('cancelado') && !statusLoc.includes('orcam')) {
              const dataStr = loc.dataRetirada ||
loc.dataEvento || loc.dataDevolucao;
              if (dataStr) {
...
// Ordena histórico do mais recente para o mais antigo
  ...
  const maskCPF = (v) => { v = v.replace(/\D/g, ""); v = v.replace(/(\d{3})(\d)/, "$1.$2"); v = v.replace(/(\d{3})(\d)/, "$1.$2");
v = v.replace(/(\d{3})(\d{1,2})$/, "$1-$2"); return v.substring(0, 14); };
  const maskCNPJ = (v) => { v = v.replace(/\D/g, "");
v = v.replace(/^(\d{2})(\d)/, "$1.$2"); v = v.replace(/^(\d{2})\.(\d{3})(\d)/, "$1.$2.$3"); v = v.replace(/\.(\d{3})(\d)/, ".$1/$2"); v = v.replace(/(\d{4})(\d)/, "$1-$2"); return v.substring(0, 18); };
  const maskPhone = (v) => { v = v.replace(/\D/g, ""); v = v.replace(/^(\d{2})(\d)/g, "($1) $2"); v = v.replace(/(\d)(\d{4})$/, "$1-$2");
return v.substring(0, 15); };
  ...
  if (clienteEditando) {
        ...
        // 🔥 MÁGICA: DISPARO DO E-MAIL DE CADASTRO MANUAL 🔥
              ...
              html: `
                <div style="font-family: sans-serif; color: #333; max-width: 600px; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; margin: 0 auto;">
                  ...
                  <div style="padding: 30px;">
                    ...
                    <p style="font-size: 16px; line-height: 1.5;">O seu cadastro foi criado e atualizado em nosso sistema com sucesso.</p>
                    ...
                    <div style="background-color: #f8fafc; border-left: 4px solid #3b82f6; padding: 15px; margin: 25px 0; border-radius: 0 8px 8px 0;">
                      ...
                      <p style="margin: 8px 0 0 0; font-size: 15px; color: #1e3a8a;">O seu perfil já está seguro com a nossa equipe. Em breve entraremos em contato para enviar o seu orçamento ou alinhar os próximos detalhes do seu evento!</p>
                    </div>

                    <p style="font-size: 16px; line-height: 1.5;">Se tiver qualquer dúvida, é só nos chamar no WhatsApp.</p>
    ...
    {fotoBase64 ? (
                <>
                  <img 
                    ...
                    style={{ objectPosition: `${posicaoFoto.x}% ${posicaoFoto.y}%`, cursor: dragging ?
  'grabbing' : 'grab', touchAction: 'none' }}
  ...
  '⏳ Cadastro Pendente' : formData.statusCadastro === 'bloqueado' ? '🚫 Bloqueado' : '✅ Cadastro Aprovado'}
                </span>
                <span className="badge-financeiro" style={{
                    backgroundColor: calculandoFinancas ?
  '#f1f5f9' : formData.situacaoFinanceira === 'inadimplente' ? '#fef2f2' : '#f0fdf4',
                    color: calculandoFinancas ?
  '#475569' : formData.situacaoFinanceira === 'inadimplente' ? '#ef4444' : '#10b981',
                    border: calculandoFinancas ?
  '1px solid #cbd5e1' : formData.situacaoFinanceira === 'inadimplente' ? '1px solid #fca5a5' : '1px solid #86efac'
                }}>
                  {calculandoFinancas ?
  '⏳ Calculando...' : (formData.situacaoFinanceira === 'inadimplente' ? '🔴 Inadimplente' : '🟢 Adimplente')}
                </span>
              </div>

              <div className="resumo-info">
                <h4>{tipoPessoa === 'fisica' ?
  (formData.nome || 'Novo Cliente') : (formData.nomeFantasia || 'Nova Empresa')}</h4>
                <p>{tipoPessoa === 'fisica' ?
  (formData.cpf || 'CPF não informado') : (formData.cnpj || 'CNPJ não informado')}</p>
              </div>

              {formData.celular && (
                <a href={`https://wa.me/55${formData.celular.replace(/\D/g, '')}`} target="_blank" rel="noreferrer" className="btn-whatsapp-resumo">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="18" height="18" fill="currentColor"><path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 
  10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7 .9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"/></svg>
                  Chamar no WhatsApp
            ...
            <div style={{background: '#f1f5f9', padding: '12px', borderRadius: '8px', color: '#0f172a', fontWeight: '800', display: 'flex', justifyContent: 'space-between', border: '1px solid #e2e8f0'}}>
                ...
                <small style={{textAlign: 'center', width: '100%', display: 'block'}}>📅 Cliente desde: {clienteEditando?.criadoEm ?
  new Date(clienteEditando.criadoEm).toLocaleDateString('pt-BR') : 'Hoje'}</small>
              </div>
            </div>
       ...
       <button type="button" className={`tab-btn ${tipoPessoa === 'fisica' ?
  'active' : ''}`} onClick={() => setTipoPessoa('fisica')}>👤 Pessoa Física</button>
              <button type="button" className={`tab-btn ${tipoPessoa === 'juridica' ?
  'active' : ''}`} onClick={() => setTipoPessoa('juridica')}>🏢 Pessoa Jurídica</button>
            </div>
         ...
         <h3 className="section-divider mt-compact">CONTROLE DE SISTEMA</h3>
            <div className="form-grid-4">
              ...
              <div className="form-group span-2">
                <label htmlFor="statusCadastro" style={{ color: formData.statusCadastro === 'pendente' ?
  '#d97706' : formData.statusCadastro === 'bloqueado' ? '#ef4444' : '#10b981', fontWeight: '800' }}>STATUS DO CADASTRO</label>
                <select id="statusCadastro" name="statusCadastro" autoComplete="off" value={formData.statusCadastro} onChange={handleChange} className="status-select" style={{ backgroundColor: formData.statusCadastro === 'pendente' ?
  '#fef3c7' : formData.statusCadastro === 'bloqueado' ? '#fef2f2' : '#f0fdf4', color: formData.statusCadastro === 'pendente' ? '#d97706' : formData.statusCadastro === 'bloqueado' ?
  '#ef4444' : '#10b981', border: formData.statusCadastro === 'pendente' ? '1px solid #fcd34d' : formData.statusCadastro === 'bloqueado' ?
  '1px solid #fca5a5' : '1px solid #86efac' }}>
  ...
  className="form-group span-2">
                <label htmlFor="situacaoFinanceira" style={{ color: calculandoFinancas ?
  '#64748b' : formData.situacaoFinanceira === 'inadimplente' ? '#ef4444' : '#10b981', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '6px' }}>SITUAÇÃO FINANCEIRA <span title="Este campo é automático">🔒</span></label>
                <select id="situacaoFinanceira" name="situacaoFinanceira" autoComplete="off" value={calculandoFinancas ?
  'calculando' : formData.situacaoFinanceira} disabled={true} className="status-select" style={{ backgroundColor: calculandoFinancas ? '#f1f5f9' : formData.situacaoFinanceira === 'inadimplente' ?
  '#fef2f2' : '#f0fdf4', color: calculandoFinancas ? '#475569' : formData.situacaoFinanceira === 'inadimplente' ? '#ef4444' : '#10b981', border: calculandoFinancas ?
  '1px solid #cbd5e1' : formData.situacaoFinanceira === 'inadimplente' ? '1px solid #fca5a5' : '1px solid #86efac', cursor: 'not-allowed', opacity: 0.9 }}>
                  ...
                  {tagsParaExibir.map(tag => {
                    ...
                    return (
                      <button key={tag} type="button" className={`tag-chip-btn ${estaSelecionada ? 'selected' : ''}`} onClick={() => selecionarTag(tag)}
                        style={estaSelecionada ? { backgroundColor: tagEstilo.bg, color: tagEstilo.color, borderColor: tagEstilo.color, boxShadow: `0 0 0 1px ${tagEstilo.color}, 0 4px 8px rgba(0,0,0,0.1)` } : { backgroundColor: tagEstilo.bg, 
  color: tagEstilo.color, borderColor: tagEstilo.border }}
                      >
                        {tag} {estaSelecionada && '✓'}
                      </button>
                    )
                ...
                {ehTagAntiga && <span style={{fontSize: '0.75rem', color: '#ef4444', display: 'block', marginTop: '8px', fontWeight: '600'}}>👻 Tag Antiga detectada: "{formData.tags}".
  Clique em uma das opções acima para atualizar.</span>}
              </div>
  ...
  'S/S').toLowerCase().replace(' ', '');
                  ...
                  <span className="h-title">
                            Pedido {loc.numeroPedido ?
  `#${loc.numeroPedido}` : `#${loc.id.substring(0,6).toUpperCase()}`}
                          </span>
                          <span className="h-date">
                            📅 Evento: {loc.dataRetirada ?
  new Date(loc.dataRetirada + 'T12:00:00').toLocaleDateString('pt-BR') : 'Sem data'}
                          </span>
                        </div>
  ...
  'S/S'}
                          ...
                          <span className="h-value" style={{textDecoration: isCancelado ?
  'line-through' : 'none', color: isCancelado ? '#94a3b8' : '#0f172a'}}>
            ...
            <div className="form-actions mt-compact" style={{gridColumn: '1 / -1'}}>
              ...
              <button type="submit" className="btn-salvar-form" disabled={salvando}>{salvando ?
  'Aguarde...' : 'Salvar Cliente'}</button>
            </div>
