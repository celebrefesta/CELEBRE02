        ...
        <div className={`modal-checkin-header ${isIda ? 'header-ida' : 'header-volta'}`}>
          <div className="header-top-line">
            <div className="header-left-badges">
              <span className="badge-modo">
                {isIda ? '📦 VISTORIA DE SAÍDA' : '🔄 VISTORIA DE DEVOLUÇÃO'}
              </span>
              <span className="header-pedido-txt">Pedido #{numeroPedido}</span>
            </div>
            <div className="header-top-actions">
              <button type="button" className="btn-header-pdf" onClick={handleGerarPDF} title="Visualizar e Imprimir Comprovante de Vistoria em PDF">
                📄 PDF
              </button>
              <button type="button" className="btn-header-wsp" onClick={handleEnviarWhatsApp} title="Compartilhar Relatório de Vistoria no WhatsApp">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="#ffffff" style={{ display: 'inline-block', verticalAlign: 'middle' }}>
              ...
              <button type="button" className="btn-close-modal" onClick={onClose} title="Fechar Vistoria">✕</button>
            </div>
          </div>
          <div className="header-bottom-line">
            ...
            <div className="header-dates-inline">
              <span className="header-date-pill">📅 Evento: <strong>{locacao.dataRetirada ? locacao.dataRetirada.split('-').reverse().join('/') : 'S/D'}</strong></span>
              {locacao.dataDevolucao && (
                <span className="header-date-pill">🔄 Devolução: <strong>{locacao.dataDevolucao.split('-').reverse().join('/')}</strong></span>
              )}
            </div>
          </div>
        </div>
