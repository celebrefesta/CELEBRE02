...
export const gerarRelatorioClientesPDF = ({
  ...
  doc.setProperties({
    title: `CARTEIRA DE CLIENTES & CRM - ${nomeEmpresa}`,
    subject: 'Relatório Executivo de Carteira de Clientes, LTV e Fidelização',
    author: nomeEmpresa,
  ...
  doc.text('CARTEIRA DE CLIENTES & CRM', 196, 12, { align: 'right' });

  doc.setFontSize(8.5);
  doc.setTextColor(255, 255, 255);
  doc.text(`SEGMENTO: ${filtroSegmento.toUpperCase()}`, 196, 18, { align: 'right' });
  ...
  doc.text(`Total: ${clientes.length} clientes filtrados`, 192, y + 4.8, { align: 'right' });
  ...
  const taxaRetorno = metricas.taxaRetorno || 0;
  const ticketMedio = metricas.ticketMedio || 0;
  const inativos = metricas.inativos || 0;
  ...
  doc.text('TOTAL DE CLIENTES', startX + 5, y + 6);
  doc.setFontSize(11);
  doc.text(`${total}`, startX + 5, y + 13);
  doc.setFontSize(6.5);
  doc.setFont('helvetica', 'normal');
  doc.text('Cadastrados na base', startX + 5, y + 18);
  ...
  doc.text('TAXA DE FIDELIDADE', x2 + 5, y + 6);
  doc.setFontSize(11);
  doc.text(`${taxaRetorno}%`, x2 + 5, y + 13);
  doc.setFontSize(6.5);
  doc.setFont('helvetica', 'normal');
  doc.text('Recompram no ano', x2 + 5, y + 18);
  ...
  doc.text('TICKET MÉDIO (LTV)', x3 + 5, y + 6);
  doc.setFontSize(10);
  doc.text(`R$ ${ticketMedio.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, x3 + 5, y + 13);
  doc.setFontSize(6.5);
  doc.setFont('helvetica', 'normal');
  doc.text('Gasto médio por pedido', x3 + 5, y + 18);
  ...
  doc.text('CLIENTES INATIVOS', x4 + 5, y + 6);
  doc.setFontSize(11);
  doc.text(`${inativos}`, x4 + 5, y + 13);
  doc.setFontSize(6.5);
  doc.setFont('helvetica', 'normal');
  doc.text('Sem locação há +6 meses', x4 + 5, y + 18);
  ...
  doc.text('LISTAGEM DETALHADA DE CLIENTES & FATURAMENTO ACUMULADO', 14, y);
  y += 3;

  const tableRows = clientes.map(c => [
    sanitizarTexto(c.nome || 'Cliente'),
    sanitizarTexto(c.telefone || 'Sem telefone'),
    sanitizarTexto(c.cidade || 'Não informada'),
    `${c.festas || 0} festas`,
    `R$ ${(c.totalGasto || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
    c.status || 'Ativo'
  ]);

  autoTable(doc, {
    head: [['Nome do Cliente', 'Contato / Telefone', 'Cidade', 'Festas', 'LTV Acumulado (R$)', 'Status']],
    body: tableRows,
    startY: y,
    theme: 'striped',
    headStyles: {
  ...
  const nomeArquivo = `Carteira_Clientes_${nomeEmpresa.replace(/[^\w\s-]/gi, '').replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
